...
internal class MultiColumnTreeView : TreeViewWithTreeModel<MyTreeElement>
{
    ...
    IOrderedEnumerable<TreeViewItem<MyTreeElement>> InitialOrder(IEnumerable<TreeViewItem<MyTreeElement>> myTypes,
        int[] history)
    {
            ...
            case MyColumns.Value3:
            {
                if (showControls)
                {
                    ...
                    if (column == MyColumns.Value1)
                        item.data.floatValue1 =
                            EditorGUI.Slider(cellRect, GUIContent.none, item.data.floatValue1, 0f, 1f);
                    if (column == MyColumns.Value2)
                        item.data.material = (Material) EditorGUI.ObjectField(cellRect, GUIContent.none,
                            item.data.material, typeof(Material), false);
