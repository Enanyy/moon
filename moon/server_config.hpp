        ...
        bool parse(const std::string& config, int32_t sid)
        {
            try
            {
                ...
                for (auto&c : doc.GetArray())
                {
					...
					std::string path = rapidjson::get_value<std::string>(&c, "path");
					std::string cpath = rapidjson::get_value<std::string>(&c, "cpath");
					auto paths = moon::split<moon::string_view_t>(path, ";");
					for (auto& v : paths)
					{
						scfg.path.emplace_back(v);
					}

					auto cpaths = moon::split<moon::string_view_t>(cpath, ";");
					for (auto &v:cpaths)
					{
						scfg.cpath.emplace_back(v);
					}
