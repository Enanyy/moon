...
int main(int argc, char*argv[])
{
	...
	printf("working_directory:%s\n", directory::working_directory.string().c_str());
    ...
    luaS_initshr();  /* init global short string table */
    {
        ...
        try
        {
			...
			lua_bind::registerlib(lua.lua_state(), "protobuf.c", luaopen_protobuf_c);
			lua_bind::registerlib(lua.lua_state(), "aoi", luaopen_aoi);
			lua_bind::registerlib(lua.lua_state(), "bit32", luaopen_bit32);
			lua_bind::registerlib(lua.lua_state(), "mysql", luaopen_mysql);
			...
			auto cpath = moon::format("package.cpath = '%s%s'", LUA_DEFAULT_CPATH.data(), LUA_CPATH_STR.data());
            lua.script(cpath);
			auto path = moon::format("package.path = '.%s;%s%s;'",LUA_PATH_STR.data(),LUA_DEFAULT_PATH.data(),LUA_PATH_STR.data());
            lua.script(path);
