...
bool lua_service::init(string_view_t config)
{
    try
    {
        ...
        {
			auto cpath = scfg.get_value<std::string_view>("cpath");
			auto cpaths = moon::split<std::string_view>(cpath, ";");
            ...
            cpaths.emplace_back(LUA_DEFAULT_CPATH);
            std::string strpath;
		...
		auto path = scfg.get_value<std::string_view>("path");
		auto paths = moon::split<std::string_view>(path, ";");
        {
            if (auto server_cfg = server_config.get_server_config(); server_cfg != nullptr)
            {
                std::copy(server_cfg->path.begin(), server_cfg->path.end(), std::back_inserter(paths));
            }
            paths.emplace_back(LUA_DEFAULT_PATH);
            std::string strpath;
        ...
        {
			auto i = luafile.find_last_of("/");
			if (i != std::string::npos)
			{
				//截取文件名，下面在文件夹（子文件夹）里找
				luafile = luafile.substr(i + 1);
			}
