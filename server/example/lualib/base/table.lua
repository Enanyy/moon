...
--根据条件函数conditionFunc安全删除item
function table.removewhere(tb,conditionfunc)
	if tb ~= nil and next(tb) ~= nil and conditionfunc ~= nil then
		for i = #tb, 1, -1 do
...
function table.contains(tb, data)
	if tb ~= nil then
		for i = #tb, 1, -1 do
			if tb[i] == data then
				return true
